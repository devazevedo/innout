<?php
    ...
    if($user->is_admin) {
        $users = User::get();
        $selectedUserId = $_POST['user'] ? $_POST['user'] : $selectedUserId; 
    }
    ...
    loadTemplateView('monthly_report', ['report' => $report, 'sumOfWorkedTime' => getTimeStringFromSeconds($sumOfWorkedTime), 'balance' => "$sign $balance", 'selectedPeriod' => $selectedPeriod, 'periods' => $periods, 'selectedUserId' => $selectedUserId, 'users' => $users]);